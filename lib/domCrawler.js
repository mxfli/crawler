...
var crawl = function (uriObj) {
  ...
  if (uriObj.type !== "link") {
    request.get({uri: uri, jar: true, encoding: null, headers: requestOptions}, function (err, response) {
    ...
    request.get({uri: uri, jar: true, encoding: null, headers: requestOptions}, function (err, res, body) {
      if (err || res.statusCode !== 200) {
        return crawlEnd(err, uriObj, res && res.statusCode || 999);
      }

      if (!Buffer.isBuffer(body)) {
        body = iconv.toEncoding(body, '');
        //console.log(iconv.fromEncoding(body,''));
        //process.exit(2);
      ...
      require('jsdom').env({ html: body, src: [jQuerySrc],
          ...
          config.crawlOptions.resourceParser.parseLinks(window.$, hander);
        }
      });
    });
  }
