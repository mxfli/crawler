...
var crawlQueue = function (baseDir, crawl) {
  ...
  that.push = function (uriObj) {
    ...
    var isNeedUpdate = function (uri) {
      if (uri.type === 'link' && link in queue.finishedStack) {
    ...
    // exit
    var isMaxFailed = (link in queue.failedStack) && queue.failedStack[link].failedCount > config.crawlOptions.maxRetryCount;
    if (isMaxFailed) {
      console.warn('MaxFailed skip push link:', link);
      return;
    }

    if (!isNeedUpdate(uriObj)) {
      console.warn('Not need update skip push link:', link);
      return;
    }

    if (uriObj.uri in queue.processingStack) {
      console.info('Processing skip push link:', link);
      return;
    }
    ...
    if (!queue.queue.some(function (e) {
      ...
      console.info('Push tu queue link:', link);
